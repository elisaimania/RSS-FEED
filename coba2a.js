...
fetch(RSS_URL)
  ...
  .then(rss =>{
    ...
    var judul1 = document.getElementById("div1");
      ...
      let tambah=``;
      items.forEach(el => {
      var img = el.description["#cdata-section"].replace('border="0"','hspace="9"');
      
        tambah += `
        <article>
          
          <h2>
            <a href="${el.guid['link']}" target="_blank" rel="noopener">
              ${el.title}
            </a>
...
fetch(RSS_URL2)
  ...
  .then(rss =>{
    ...
    items.forEach(el => {
      ...
      var img = el.description["#text"].replace('hspace="7"','hspace="9"');
        
      tambah += `
        <article>
          
          <h2>
            <a href="${el.enclosure['link']}" target="_blank" rel="noopener">
              ${title}
            </a>
